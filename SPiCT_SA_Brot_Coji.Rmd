...
## Sardina austral Aysén


```{r}
sardAustral <- read.table("data_SardAustralAysen.txt", sep="", header = T)
```

Convierto los datos como lista para ser lidos por las fiunciones del SPiCT
```{r}
sardAustral  <- as.list(sardAustral)
#compruebo
class(sardAustral)
```
...
Primero un vistazo a mis datos y compruebo el contraste, el que desde tya nos di´ra que tan robusta es la estimación
```{r}
plotspict.data(sardAustral)
plotspict.ci(sardAustral)
```
ahora aplico el modelo
```{r}
ressard <- fit.spict(sardAustral)
plotspict.biomass(ressard)
plotspict.catch(ressard)
plotspict.f(ressard)
plotspict.diagnostic(ressard)
plotspict.fb(ressard)
plotspict.bbmsy(ressard)
plotspict.production(ressard)
...
```{r}
list.quantities(ressard)
```

Ahora los saco por separado
```{r}
get.par('MSY', ressard)
get.par('Fmsy', ressard)
get.par('Bmsy', ressard)
get.par('BmsyB0', ressard)
```
