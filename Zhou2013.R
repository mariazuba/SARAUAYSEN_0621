...
#          S. Zhou, Modificado por Elson Leal
#             para sardina austral Aysen 2019, CBA 2020               #
############################################################# 

# This method requires time series of catch data only. 
# However, some life history parameters, M, Linf, k, T_max, T_maturation, 
# will improve the performance. Also, a rough guess of maximum depletion level D = B_end/K will be helpful.
# This example is for single catch series
# ----------------------------------------------------------------------------
...
sim1=function(k25=k25,k75=k75,r25=r25,r75=r75,yr=yr,C=C,nsim=nsim,msy=NULL){ 
 ...
 y1<-seq(0,round(k75,0)*1.1,le=5);x1<-seq(1,length(yr),by=1)
 axis(1,at=yr[x1],labels=yr[x1],las=1)
 axis(2,at=y1,labels=format(round(y1/1000,0),3),las=2)
 legend(yr[nyr-2],max(y1)*1.05,c("Biomasa","Captura","RMS"),lty=c(1,2,2),
 col=c(1,"#363636",2),pch=c(NA,21,NA),lwd=c(2,2,2),pt.bg=c(NA,4,NA),cex=1.3)
 BC<-expression(paste('Biomasa ('%*%'1000 ton) y Captura (ton)',sep="")) 
 mtext(BC,side=2,line=4.0,cex=1.3)
 mtext("Años",side=1,line=3.5,cex=1.3)
...
dep=round(seq(0.1,0.8,0.05),2);nd=length(dep)
...
par(mar=c(5.2,6.2,4,4),cex.axis=1.3,cex.lab=1.3)
...
quan1.B=apply(B2Bmsy,1,quant)[1,],quan3.B=apply(B2Bmsy,1,quant)[2,],
F2Fmsy.med=apply(F2Fmsy,1,median),quan1.F=apply(F2Fmsy,1,quant)[1,],
...
library(xlsx)
write.xlsx(tabla,"tabla1.xlsx",row.names=FALSE) 
...
#
soli<-cbind(yr,Bioma[3,],Fest,X1,Y1)
colnames(soli)<-c("year","BD","Fest","Brel","Frel")
#library(rJava)                                         ## PROBLEMAS CON ESTO
#library(xlsx)
write.xlsx(soli,"tabla2.xlsx",row.names=FALSE) 
#DEFINE LAS AREAS DE LOS POLIGONOS
cols<-c("#696969","#A8A8A8","#DEDEDE")
ini<-c(-0.09);xmax<-max(X1)*1.35;ymax<-max(Y1)*1.22;xmin<-(0.08)
...
lines(X1,Y1,type="p",bg=2,pch=21,cex=1.5,lwd=2)
...
text(0.5,max(Y1)*1.29,expression(PBR[paste("agotamiento")]),cex=1.6,
pos=1,xpd=T)
text(1,max(Y1)*1.29,expression(B[paste("RMS")]),cex=1.6,pos=1,xpd=T)
text(max(X1)*1.19,1.11,expression(F[paste("RMS")]),cex=1.6,pos=1,xpd=T)
mtext(expression(paste("Biomasa total relativa a ",B[paste("RMS")],sep="")),
side=1,line=4.2,cex=1.8)
mtext(expression(paste("Mortalidad por pesca relativa a ",F[paste("RMS")],
sep="")),side=2,line=3.8,cex=1.8)
#a2<-which(yr%in%seq(2015,1994,by=-2)==TRUE)
a2<-c(1,2,3,4,5,6,7,8,9)
text(X1[a2],Y1[a2]*1.06,yr[a2],cex=1.3,pos=4,col=4)
...
yy<-length(yr);prob=c(0.75,1.00,1.25)
ss<-dim(sp);yrs_pro<-seq(yr[yy],yr[yy]+11,by=1);yyp<-length(yrs_pro)
BT=CT=array(NA,c(length(prob),ss[1],length(yrs_pro)))
BT[,,1]<-Bioma[3,yy];CT[,,1]<-C[yy]
Fi=Fmrs*prob
for(y in 1:length(Fi)){
 for(n in 1:ss[1]){
  ki=sp$k[n];ri=sp$r[n]
  for(m in 1:(yyp-1)){
   if(m==1){CT[y,,1]=C[yy]} else {CT[y,n,m]=BT[y,n,m]*Fi[y]}
   BT[y,n,m+1]=BT[y,n,m]+ri*BT[y,n,m]*(1-BT[y,n,m]/ki)-CT[y,n,m]
   if(m==(yyp-1)){CT[y,n,m+1]=BT[y,n,m+1]*Fi[y]}
  }
 }
}
...
plot(yrs_pro[-2],seq(0,rng1[2]*1.15,le=(y1-1)),col=0,type="l",xlab="",ylab="",yaxt="n",ylim=c(0,rng1[2]*0.98))
for(x in 1:length(Fi)){
 lines(yrs_pro[-2],apply(BT[x,,],2,quantile)[3,-2],col=x,lty=2,lwd=2)
}
abline(h=Bmrs,lty=2,lwd=2,col=cols)
axis(2,at=ax2,labels=ax2/1000,las=1,cex=1.4)
mtext(BB,side=2,line=3.5,cex=1.4)
mtext("Años",side=1,line=3.4,cex=1.4)
legend(2021,rng2[2]*0.5,c(expression(F[paste("0.35")]),expression(F[paste("0.46")]),
...
#x11(width=8,height=7)
par(mar=c(6.1,6.2,4.8,4.8),cex.axis=1.4,cex.lab=1.4)
plot(yrs_pro[-2],seq(0,rng2[2]*1.15,le=11),col=0,type="l",xlab="",ylab="",yaxt="n",ylim=c(0,rng2[2]*0.95))
for(x in 1:length(Fi)){
 lines(yrs_pro[-2],apply(CT[x,,],2,quantile)[3,-2],col=x,lty=2,lwd=2)
}
abline(h=MRS,lty=2,lwd=2,col=cols)
axis(2,at=ax3,labels=sprintf("%0.0f",ax3/1000),las=1,cex=1.4)
mtext(CC,side=2,line=3.5,cex=1.4)
mtext("AÑos",side=1,line=3.4,cex=1.4)
legend(2020,rng2[2]*0.5,c(expression(F[paste("0.35")]),expression(F[paste("0.46")]),
expression(F[paste("0.58")])),lty=c(2,2,2,2,2),lwd=c(2,2,2,2,2),cex=1.4,
col=c(1,2,3))
dev.off()
