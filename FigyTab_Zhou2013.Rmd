...
colnames(soli)<-c("year","BD","Fest","B_Bo","Brel","Frel")
...
for(y in 1:length(Fi)){
 for(n in 1:ss[1]){
  ...
  ri=sp$r[n]
  for(m in 1:(yyp-1)){
   if(m==1){ # igual a la captura del último año
     CT[y,,1]=C[yy]} 
    else {
      CT[y,n,m]=BT[y,n,m]*Fi[y]}
